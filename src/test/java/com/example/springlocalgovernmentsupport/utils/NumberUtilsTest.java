...
class NumberUtilsTest {
    ...
    public void convertStringToNumber() {
        ...
        assertEquals(0, NumberUtils.convertStringToNumber("추천금액 이내"));
    }

    @Test
    public void convertStringToFromRateAndEndRate() {
        assertEquals(3f, NumberUtils.convertStringToFromRateAndEndRate("3%").get(0));

        List<Float> float1 = NumberUtils.convertStringToFromRateAndEndRate("3%~5%");
        assertEquals(3f, float1.get(0));
        assertEquals(5f, float1.get(1));

        List<Float> float2 = NumberUtils.convertStringToFromRateAndEndRate("2.5%~5.0%");
        assertEquals(2.5f, float2.get(0));
        assertEquals(5.0f, float2.get(1));

        List<Float> float3 = NumberUtils.convertStringToFromRateAndEndRate("1.5%~3.86%");
        assertEquals(1.5f, float3.get(0));
        assertEquals(3.86f, float3.get(1));
